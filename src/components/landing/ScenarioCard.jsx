...
export default function ScenarioCard({ icon, text, dark = false }) {
  if (dark) {
    return (
      <div className="relative overflow-hidden rounded-xl border border-zinc-700 bg-zinc-800/90 p-4 ring-1 ring-white/[0.04]">
        ...
        <div className="relative flex items-start gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-zinc-900 ring-1 ring-orange-500/25">
            <Icon name={icon} size={20} className="text-orange-400" strokeWidth={1.75} />
          </div>
          <p className="pt-1.5 text-sm leading-snug text-zinc-200">{text}</p>
        </div>
      </div>
    )
  }
